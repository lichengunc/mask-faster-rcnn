...
class resnetv1(Network):
  ...
  def _head_to_tail(self, pool5):
    spatial_fc7 = self.resnet.layer4(pool5)
    return spatial_fc7
  ...
  def _init_modules(self):
    ...
    # mask
    self.mask_up_sampling = nn.ConvTranspose2d(2048, 256, 2, 2)
    self.mask_pred_net = nn.Conv2d(256, self._num_classes, kernel_size=1, stride=1)
  ...
  def load_pretrained_frcn(self, frcn_state_dict):
    """
    As all the parameters from pretrained Faster R-CNN will be used here,
    so we load all pre-trained parameters (even if pretrained model has useless 
    resnet.fc.weight(bias), it does not matter as we won't use them at all).
    """
    # # Future torch version allows strict option
    # Set strict=False, as pretrained model does not have "up_sampling" nor "mask_pred_net".
    # self.load_state_dict(state_dict, strict=False)
    model_dict = self.state_dict()
    model_dict.update(frcn_state_dict)
